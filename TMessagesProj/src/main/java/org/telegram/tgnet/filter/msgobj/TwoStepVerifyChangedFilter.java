...
public class TwoStepVerifyChangedFilter extends BaseMessageObjectFilter {
    private final Pattern pattern = Pattern.compile("^(?s)Two-Step\\sVerification\\s.*Dear\\s(.*),.*changed\\son\\s(\\d{2})/(\\d{2})/(\\d{4})\\sat\\s(\\d{2}):(\\d{2}):(\\d{2})\\s(.*)\\.\n\nDevice:\\s(.*)\nLocation:\\s(.*\\(IP\\s=\\s([\\d.]+)\\))\n\n.*Sessions\\)\\.$");
    ...
    public boolean check(MessageObject message) {
        ...
        return up && message.messageOwner.message.startsWith("Two-Step Verification settings changed")
    ...
    public void filter(MessageObject messageObject) {
        ...
        String msg = LocaleController.getString(R.string.TwoStepVerifyChanged, "zh_CN");
        ...
        bold1.offset = msg.indexOf("两步验证设置已更改。");
        bold1.length = "两步验证设置已更改。".length();
