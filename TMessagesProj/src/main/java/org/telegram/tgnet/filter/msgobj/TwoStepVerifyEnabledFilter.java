...
public class TwoStepVerifyEnabledFilter extends BaseMessageObjectFilter {
    private final Pattern pattern = Pattern.compile("^(?s)Two-Step\\sVerification\\s.*Dear\\s(.*),.*account\\son\\s(\\d{2})/(\\d{2})/(\\d{4})\\sat\\s(\\d{2}):(\\d{2}):(\\d{2})\\s(.*)\\.\n\nDevice:\\s(.*)\nLocation:\\s(.*\\(IP\\s=\\s([\\d.]+)\\))\n\n.*Sessions\\)\\.$");
    ...
    public boolean check(MessageObject message) {
        ...
        return up && message.messageOwner.message.startsWith("Two-Step Verification enabled")
    ...
    public void filter(MessageObject messageObject) {
        ...
        String msg = LocaleController.getString(R.string.TwoStepVerifyEnabled, "zh_CN");
        ...
        bold1.offset = msg.indexOf("两步验证已启用。");
        bold1.length = "两步验证已启用。".length();
